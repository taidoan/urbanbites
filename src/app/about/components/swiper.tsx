...
const CardContent = ({
  ...
  <Card event={event} review={review}>
    {event && slide.date && (
      <CardDate day={slide.date.day} month={slide.date.monthText} />
    )}
    {insideImage && slide.image && !review ? (
      ...
      <CardBody>
        <Review
          rating={slide.review.rating}
          author={slide.review.author}
          date={slide.review.date}
        >
          <p>{slide.content}</p>
        </Review>
      </CardBody>
...
const SwiperSection = ({
  ...
  useEffect(() => {
    const shouldInit =
      name === "events" || name === "locations" ? !isSmaller : !isDesktop;
